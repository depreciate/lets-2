...
def firstPlace(action,fileMd5,userID, gameMode, mods):
	if action == True:
		glob.db.execute("INSERT INTO users_firstplaces (user,  time, game_mode, beatmap_md5, mods) VALUES (%s, %s, %s, %s, %s)",[userID, int(time.time()), gameMode, fileMd5, mods])
	else:
		glob.db.execute("DELETE FROM users_firstplaces WHERE user = %s AND game_mode = %s AND beatmap_md5 = %s LIMIT 1",[userID, gameMode, fileMd5])
	return True
